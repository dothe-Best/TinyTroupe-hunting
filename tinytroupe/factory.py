...
class TinyPersonFactory(TinyFactory):
    ...
    def generate_person(self, agent_particularities:str=None, temperature:float=1.5, attempts:int=5):
        ...
        def aux_generate():
            ...
            messages = []
            messages += [
                {"role": "system", "content": "You are a system that generates specifications of artificial entities."},
            ...
            if message is not None:
                logger.debug(f"LLM 返回的内容：{message['content']}")
                result = utils.extract_json(message["content"])

                logger.debug(f"解析后的代理规范：{result}")
                try:
                    if result["name"].lower() not in self.generated_names:
                        return result
                except KeyError as e:
                    logger.error(f"生成的代理规范缺少必需的字段：{e}")
                    logger.debug(f"完整的代理规范：{result}")
