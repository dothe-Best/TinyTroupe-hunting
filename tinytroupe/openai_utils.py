...
###########################################################################
default = {}
default["model"] = config["OpenAI"].get("MODEL", "gpt-4o-2024-08-06")
...
class OpenAIClient:
    ...
    def _setup_from_config(self):
        ...
        api_base = config["OpenAI"].get("API_BASE", "https://api.openai.com/v1")

        if not api_key:
            logger.error("API_KEY for OpenAI is missing.")
            raise ValueError("API_KEY for OpenAI is missing.")

        self.client = openai
        self.client.api_key = api_key
        self.client.api_base = api_base  # 设置自定义 API 基地址
